...
void PlayersMain::FillProfileWithInfo()
{
	...
	this->sPlayerProfile->wCurLP = db->getInt("CurLP");
...
void PlayersMain::SetPlayerDamage(int value)
{
	...
	if (this->GetPcProfile()->wCurLP - value <= 0)
		this->GetPcProfile()->wCurLP = 0;
	else
		this->GetPcProfile()->wCurLP -= value;
}
/*
...
void PlayersMain::SavePlayerData(CGameServer* app)
{
	...
	db->setInt(12, this->GetPcProfile()->wCurLP);
	db->setInt(13, this->GetPcProfile()->wCurEP);
	db->setInt(14, this->GetPcProfile()->wCurRP);
	//db->setInt(15, this->GetPcProfile()->wCurAp);
...
void PlayersMain::SetLevelUP()
{
	...
	*/
	...
	this->GetPcProfile()->wCurLP = this->GetPcProfile()->avatarAttribute.wBaseMaxLP;
...
void PlayersMain::SendThreadUpdateEPLP()
{
	...
	res->handle = this->avatarHandle;
	res->wCurEP = this->GetPcProfile()->wCurEP;
	res->wCurLP = this->GetPcProfile()->wCurLP;
...
void PlayersMain::SendThreadUpdateOnlyLP()
{
	...
	{
		this->GetPcProfile()->wCurLP += this->GetPcProfile()->avatarAttribute.wBaseLpRegen; // += regen
		if (this->GetPcProfile()->wCurLP > this->GetPcProfile()->avatarAttribute.wBaseMaxLP)
			this->GetPcProfile()->wCurLP = this->GetPcProfile()->avatarAttribute.wBaseMaxLP;
	}
		...
		while (true)
		{
			if (plr->GetPlayerDead() == false)
			{
				...
				else if (plr->GetPlayerFight() == false)
				{
					...
					{
						if (plr->GetPcProfile()->wCurLP < plr->GetPcProfile()->avatarAttribute.wBaseMaxLP || plr->GetPcProfile()->wCurLP > plr->GetPcProfile()->avatarAttribute.wBaseMaxLP)
