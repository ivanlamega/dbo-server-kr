...
//--------------------------------------------------------------------------------------//
//  Set player stats and attributes
//--------------------------------------------------------------------------------------//
void CClientSession::SetPlayerAttributeInfo(CGameServer * app, sPC_TBLDAT * pTblData)
{
	...
	db->setInt(1, this->plr->pcProfile->charId);
	db->execute();
	db->fetch();

	this->plr->pcProfile->avatarAttribute.byBaseStr = pTblData->byStr;
	this->plr->pcProfile->avatarAttribute.byLastStr = app->db->getInt("LastStr");
...
void CClientSession::CheckPlayerStat(CGameServer * app, sPC_TBLDAT *pTblData, int level)
{
	...
	app->db->setInt(1, pTblData->byStr + ((BYTE)pTblData->fLevel_Up_Str));
	app->db->setInt(2, pTblData->byCon + ((BYTE)pTblData->fLevel_Up_Con));
	app->db->setInt(3, pTblData->byFoc + ((BYTE)pTblData->fLevel_Up_Foc));
	app->db->setInt(4, pTblData->byDex + ((BYTE)pTblData->fLevel_Up_Dex));
	app->db->setInt(5, pTblData->bySol + ((BYTE)pTblData->fLevel_Up_Sol));
	app->db->setInt(6, pTblData->byEng + ((BYTE)pTblData->fLevel_Up_Eng));
	app->db->setInt(7, this->plr->pcProfile->charId);
	app->db->execute();
	...
	app->db->prepare("UPDATE characters SET BaseAttackRange = ?, LastAttackRange = ? WHERE CharID = ?");
	app->db->setFloat(1, pTblData->fAttack_Range);
	app->db->setFloat(2, pTblData->fAttack_Range);
...
void CClientSession::SendAvatarCharInfo(CNtlPacket * pPacket, CGameServer * app)
{
	...
	res->sPcProfile.avatarAttribute.fBaseAttackRange = pTblData->fAttack_Range;
	...
	res->sPcProfile.avatarAttribute.wLastPhysicalOffence = ((pTblData->byStr) + (app->db->getInt("LastPhysicalOffence")));
	res->sPcProfile.avatarAttribute.wLastPhysicalDefence = ((pTblData->byDex) + (app->db->getInt("LastPhysicalDefence")));
	res->sPcProfile.avatarAttribute.wLastEnergyOffence = ((pTblData->byFoc) + (app->db->getInt("LastEnergyOffence")));
	res->sPcProfile.avatarAttribute.wLastEnergyDefence = (pTblData->bySol + app->db->getInt("LastEnergyDefence"));
	res->sPcProfile.avatarAttribute.wLastAttackRate = pTblData->byFoc + app->db->getInt("LastAttackRate");
	res->sPcProfile.avatarAttribute.wLastDodgeRate = pTblData->byDex + app->db->getInt("LastDodgeRate");
	res->sPcProfile.avatarAttribute.wLastBlockRate = pTblData->byDex + app->db->getInt("LastBlockRate");
	res->sPcProfile.avatarAttribute.wLastPhysicalCriticalRate = pTblData->byDex + app->db->getInt("LastPhysicalCriticalRate");
	res->sPcProfile.avatarAttribute.wLastEnergyCriticalRate = pTblData->byFoc + app->db->getInt("LastEnergyCriticalRate");
	res->sPcProfile.avatarAttribute.fLastAttackRange = pTblData->fAttack_Range;
	res->sPcProfile.avatarAttribute.fLastRunSpeed = (float)app->db->getDouble("LastRunSpeed");
	res->sPcProfile.avatarAttribute.wLastMaxLP = pTblData->wBasic_LP + app->db->getInt("LastMaxLP");
	res->sPcProfile.avatarAttribute.wLastMaxRP = app->db->getInt("LastMaxRP");
	res->sPcProfile.avatarAttribute.wLastMaxEP = pTblData->wBasic_EP + app->db->getInt("LastMaxEP");
...
void CClientSession::SendCharActionAttack(RwUInt32 uiSerialId, RwUInt32 m_uiTargetSerialId, CNtlPacket * pPacket)
{
	...
	if (pTblData->byClass == 0 || pTblData->byClass == 3 || pTblData->byClass == 5){
		formula = this->plr->pcProfile->avatarAttribute.wLastPhysicalOffence;
		res->wAttackResultValue = formula;
	}
	else if (pTblData->byClass == 1 || pTblData->byClass == 4 || pTblData->byClass == 6){
		formula = this->plr->pcProfile->avatarAttribute.wLastEnergyOffence;
		res->wAttackResultValue = formula;
	}
...
void	CClientSession::SendMobLoot(CNtlPacket * pPacket, CGameServer * app, RwUInt32 m_uiTargetSerialId)
{
	...
	{
		//int droprate = rand() % 18 + 1;
		sMOB_TBLDAT* mob = (sMOB_TBLDAT*)app->g_pTableContainer->GetMobTable()->FindData(mobid);
		//sNORMAL_DROP_TBLDAT * NDropTbl = (sNORMAL_DROP_TBLDAT*)app->g_pTableContainer->GetNormalDropTable()->FindData(mob->drop_Item_Tblidx)->tblidx;
		printf("Each rate control %d\n", mob->byDropEachRateControl);
		printf("Drop Each Item %d\n", mob->byDropEItemRateControl);
		...
		sBASIC_DROP_TBLDAT * bTblData = (sBASIC_DROP_TBLDAT *)bdrop->FindData(mob->drop_Item_Tblidx);
		CEachDropTable * edrop = app->g_pTableContainer->GetEachDropTable();
		sEACH_DROP_TBLDAT * eTblData = (sEACH_DROP_TBLDAT *)edrop->FindData(mob->dropEachTblidx);
		CNormalDropTable * ndrop = app->g_pTableContainer->GetNormalDropTable();
		sNORMAL_DROP_TBLDAT * nTblData = (sNORMAL_DROP_TBLDAT*)ndrop->FindData(mob->drop_Item_Tblidx);
		CSuperiorDropTable * sdrop = app->g_pTableContainer->GetSuperiorDropTable();
		sSUPERIOR_DROP_TBLDAT * sTblData = (sSUPERIOR_DROP_TBLDAT *)sdrop->FindData(mob->drop_Item_Tblidx);
		CLegendaryDropTable * ldrop = app->g_pTableContainer->GetLegendaryDropTable();
		sLEGENDARY_DROP_TBLDAT * lTblData = (sLEGENDARY_DROP_TBLDAT *)ldrop->FindData(mob->drop_Item_Tblidx);
		CDragonBallTable * ddrop = app->g_pTableContainer->GetDragonBallTable();
		sDRAGONBALL_TBLDAT * dTblData = (sDRAGONBALL_TBLDAT *)ddrop->FindData(11);
		...
		if ( 1 == 1 )
		{
			//printf("Dragon Ball TBLIDX %d\n", dTblData->aBallTblidx[0]);
			//printf("Normal Drop Tblidx %d\n", nTblData->aItem_Tblidx[0]);
			res2->handle = AcquireSerialId();
			res2->sObjectInfo.objType = OBJTYPE_DROPITEM;
			res2->sObjectInfo.itemBrief.tblidx = 19905;
			res2->sObjectInfo.itemState.bIsNew = true;
			sVECTOR3 mypos = this->plr->GetPosition();
			res2->sObjectInfo.itemState.vCurLoc.x = mypos.x - 1;
			res2->sObjectInfo.itemState.vCurLoc.y = mypos.y;
			res2->sObjectInfo.itemState.vCurLoc.z = mypos.z - 1;
...
//CHAR_TOGG_FIGHTING
	...
	if (pBattleData->m_uiTargetSerialId)
		res->handle = pBattleData->m_uiTargetSerialId;
...
void CClientSession::SendItemPickUpReq(CNtlPacket *pPacket, CGameServer * app)
{
	...
	CNtlPacket packet2(sizeof(sGU_OBJECT_DESTROY));
	sGU_OBJECT_DESTROY * res2 = (sGU_OBJECT_DESTROY *)packet2.GetPacketData();

	CItemTable * pItemTable = app->g_pTableContainer->GetItemTable();
	sITEM_TBLDAT * itemTBL = reinterpret_cast<sITEM_TBLDAT*>(pItemTable->FindData(app->db->getInt("tblidx")));

	res->wOpCode = GU_ITEM_PICK_RES;
	res->itemTblidx = itemTBL->tblidx;
	...
	app->db->prepare("INSERT INTO items (`tblidx`, `owner_id`, `place`, `pos`, `count`, `rank`, `durability`, `grade`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);");
	app->db->setInt(1, itemTBL->tblidx);
	app->db->setInt(2, this->plr->pcProfile->charId);
	app->db->setInt(3, 0);
	app->db->setInt(4, 1);
	app->db->setInt(5, 1);
	...
	res2->wOpCode = GU_OBJECT_DESTROY;
	res2->handle = req->handle;
	packet2.SetPacketLen(sizeof(sGU_OBJECT_DESTROY));
	g_pApp->Send(this->GetHandle(), &packet2);
